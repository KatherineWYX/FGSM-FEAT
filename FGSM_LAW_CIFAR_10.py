...
def get_args():
    ...
    parser.add_argument('--lam_scale', default=0.12, type=float, help='Label Smoothing Scale')
    parser.add_argument('--lam_max', default=45, type=float, help='lambda maximum')
    parser.add_argument('--lam_start', default=50, type=int, help='Label Smoothing time')
    # TDAT
    parser.add_argument('--gamma', default=0.15, type=float, help='Label relaxation factor')
    parser.add_argument('--inner_gamma', default=0.15, type=float, help='Label relaxation factor')
    parser.add_argument('--beta', default=0.5, type=float)
    parser.add_argument('--mu', default=3.5, type=float)
...
def main():
    ...
    output_path = os.path.join(output_path, 'scale_' + str(args.lam_scale))
    output_path = os.path.join(output_path, 'time_' + str(args.lam_start))
    ...
    if args.lr_schedule == 'cyclic':
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=[lr_steps * 109 / 120, lr_steps * 114 / 120],
                                                         gamma=0.1)
    ...
    momentum = torch.zeros(args.batch_size, 3, 32, 32).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        # inner_gammas = args.beta / (1 + math.exp(-(1 - args.mu * epoch / args.epochs)))
        ...
        for i, (X, y) in enumerate(tqdm(train_loader, desc="Training Progress", unit="batch")):
            ...
            if X.shape[0] == args.batch_size:
                ...
                loss = F.cross_entropy(adv_output, relaxtion_label.float())
                ...
                loss = LabelSmoothLoss(ori_output, label_smoothing.float()) \
                       + lam_reg * (
                               loss_fn(output.float(), adv_output.float()) +
                               loss_fn(fea_output.float(), ori_fea_output.float())
                       ) / (loss_fn((X + delta[:X.size(0)]).float(), X.float()) + 0.125)
                opt.zero_grad()
                # with amp.scale_loss(loss, opt) as scaled_loss:
                loss.backward()
                ...
                epoch_train_clean_list.append(clean_acc)
                epoch_train_pgd_list.append(adv_acc)
                train_n += y.size(0)
        ...
        clean_acc_epoch = sum(epoch_train_clean_list) / len(epoch_train_clean_list)  # clean 平均精度
        adv_acc_epoch = sum(epoch_train_pgd_list) / len(epoch_train_pgd_list)

        if epoch > args.lam_start:  # 50 轮以后才开始调整 lam_reg
            # lambda_scale = 0.15  # 新增超参，控制增长速度（推荐 0.3~0.7）
            lam_reg = lam_reg * torch.exp(
                args.lam_scale * torch.tensor((clean_acc_epoch - adv_acc_epoch) / clean_acc_epoch, device=X.device)
            ).item()
            lam_reg = min(max(lam_reg, args.lamda), args.lamda * 1.5)
            print(lam_reg)

        # 清空 list，防止累积到下一个 epoch
        epoch_train_clean_list.clear()
        epoch_train_pgd_list.clear()
        ...
        model_test.load_state_dict(teacher_model.model.state_dict(), strict=False)
        model_test.float()
        model_test.eval()
