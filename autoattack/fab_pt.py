...
class FABAttack():
    ...
    def projection_linf(self, points_to_project, w_hyperplane, b_hyperplane):
        ...
        u = torch.arange(0, w.shape[0]).to(device)
        ...
        # 确保所有新创建的张量都在正确设备上
        lb = torch.zeros(c2.shape[0]).to(device)
        ub = torch.ones(c2.shape[0]).to(device) * (w.shape[1] - 1)
        nitermax = torch.ceil(torch.log2(torch.tensor(w.shape[1]).float())).to(device)
        counter2 = torch.zeros(lb.shape).long().to(device)
        ...
        while counter < nitermax:
            ...
            # 确保索引和目标张量在相同设备上
            ind3 = ind3.to(device)
            ind32 = ind32.to(device)
            counter4 = counter4.to(device)
